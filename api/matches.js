...
function filterAuthorizedStreams(matches) {
  ...
  return (matches || []).map((match) => ({
    ...match,
    servers: allowed.length
      ? (match.servers || []).filter((server) => {
          try {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    return res.status(200).json({ ...data, matches: filterAuthorizedStreams(data.matches) });
  } catch {
